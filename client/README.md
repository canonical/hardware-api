...
it for noble distro, but you can replace it with the desired one:
...
`noble` with a corresponding release):
...
```
qemu-img resize autopkgtest-noble-amd64.img +15G
```
...
autopkgtest \
  ...
  -- qemu --ram-size 4096 /var/lib/adt-images/autopkgtest-noble-amd64.img
```
